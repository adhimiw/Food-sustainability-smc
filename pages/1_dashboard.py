...
from database.db import get_db, init_database, query_df
...
def page_forecast():
    ...
    if train_btn:
        ...
        overview = query_df(f"""
            ...
            FROM sales WHERE 1=1 {date_filter_plain}
...
def page_cascade():
    ...
    if st.button("🔄 Run Cascade Optimization", type="primary", use_container_width=True):
...
def page_analytics():
    ...
    with tab2:
        ...
        st.dataframe(store_detail.sort_values("waste_rate"), use_container_width=True, hide_index=True)

    with tab3:
        st.subheader("Product Waste Analysis by Shelf Life")
        prod_analysis = query_df(f"""
